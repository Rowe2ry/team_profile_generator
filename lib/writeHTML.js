...
const writeHTML = (arr) => {
    ...
    <head>
        ...
        <link rel="stylesheet" href="./assets/${arr[0].name[0]}_style.css">
    ...
    fs.writeFile(`./src/${arr[0].name[0]}_team.html`, wholePage, err => {
        if (err) {
            throw (err);
        } else {
            console.log(`New file "${arr[0].name[0]}_team.html" created!`);
            process.exit();
        };
    });
};
