...
const writeHTML = (arr) => {
    ...
    const firstName = arr[0].name.trim().split(' ');
    ...
    for (employee of arr) {
        ...
        if (employee.constructor.name === 'Manager') {
            ...
            specialFormatting = `<li class="attribute">GitHub: <a href="https://www.github.com/${employee.github}" target="_blank">${employee.github}</a></li>`;
        } else {
    ...
    <head>
        ...
        <link rel="stylesheet" href="./assets/${firstName[0]}_style.css">
    ...
    fs.writeFile(`./dist/${firstName[0]}_team.html`, wholePage, err => {
        if (err) {
            throw (err);
        } else {
            console.log(`New file "${firstName[0]}_team.html" created!`);
            process.exit();
        };
    });
};
